...
public abstract class AbstractGA<G extends Number, F> {
	...
	public Map<String, Object> solve(Double maxTime, Integer refCost, Integer smallCost) {
		 ...
		 */
		for (int g = 1; g <= generations && totalTime <= maxTime; g++) {
			...
			if (fitness(bestChromosome) > bestSol.cost) {
				...
				// Reference cost check.
				if(bestSol.cost > smallCost && !pastSmall) {
					targetTime = (double) (System.currentTimeMillis() - startTime) / (double) 1000;
					pastSmall = true;
				}
				if(bestSol.cost > refCost) break;
			}
		...
		log.put("objectiveFunction", bestSol.cost);
